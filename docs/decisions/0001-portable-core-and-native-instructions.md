...
manifest profiles match the `.agents` directories: `tools`, `hooks`, and
`skills`. Copilot CLI, Codex, and Claude Code are the first adapter targets;
...
may create an owned import bridge. The hooks profile enters 1.0 only as
command-hook projection; permissions, models, and subagents still require a
